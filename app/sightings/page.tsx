...
export default function SightingsPage() {
  ...
  const sorted = useMemo(() => {
    ...
    if (sort === 'credibility') {
      return base.sort(
        (a, b) =>
          Number.parseFloat(b.credibilityScore) -
          Number.parseFloat(a.credibilityScore),
      )
    }
      ...
      >
        ...
        {tab === 'recent' && (
          <button
            ...
            onClick={() =>
              setSort((value) =>
                value === 'recent' ? 'credibility' : 'recent',
              )
            }
          >
      ...
      <div style={{display: 'flex', gap: 8, marginBottom: 14}}>
        {(['recent', 'trending', 'convoys'] as const).map((value) => (
          <button
            ...
            style={
              tab === value
                ? {borderColor: 'rgba(62, 168, 255, 0.8)', color: 'white'}
                : undefined
            }
          >
            {value}
            ...
            <article key={item.id} className='panel' style={{padding: 12}}>
              ...
              >
                <span className='badge'>
                  Credibility{' '}
                  {Number.parseFloat(item.credibilityScore).toFixed(0)}%
                </span>
                <span className='badge'>
                  Votes {item.upvotes + item.downvotes}
                </span>
                {item.agencyType ? (
                  <span className='badge'>{item.agencyType}</span>
                ) : null}
              </div>
          ...
          {(trending ?? []).map((item, index) => (
            ...
            >
              ...
              <p className='muted' style={{marginBottom: 8}}>
                Last seen {new Date(item.lastSeen).toLocaleString()} · Avg
                credibility {item.avgCredibility.toFixed(1)}%
              </p>
              <Link
      ...
      {tab === 'convoys' && (
        <div className='grid'>
          {(convoys ?? []).map((item, index) => (
            <article
              key={`${item.centerLat}-${item.centerLon}-${index}`}
              className='panel'
              style={{padding: 12}}
            >
              <strong>{item.vehicleCount} vehicles moving together</strong>
              <p className='muted'>
                Window {new Date(item.firstSeen).toLocaleTimeString()} -{' '}
                {new Date(item.lastSeen).toLocaleTimeString()}
              </p>
              <p className='muted'>Plates: {item.plates.join(', ')}</p>
              <p className='muted'>
                Agencies: {item.agencies.join(', ') || 'unknown'}
              </p>
            </article>
          ))}
          {(convoys ?? []).length === 0 ? (
            <p className='muted'>No active convoy detections.</p>
          ) : null}
        </div>
      )}
    </section>
